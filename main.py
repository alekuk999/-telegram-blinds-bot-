...
def generate_post_with_yandex_gpt(product_name, product_description):
    ...
    headers = {"Content-Type": "application/json", "Authorization": f"Bearer {YANDEX_API_KEY}"}
    prompt = f"""
    Ты — маркетолог интернет-магазина жалюзи в Астрахани.
    Создай цепляющий пост для Telegram на основе описания товара.

    Товар: {product_name}
    Описание: {product_description}

    Пост должен включать:
    - Яркий заголовок (до 50 символов)
    - Текст (до 200 символов, с эмодзи)
    - 3 хэштега
    - Призыв к действию
...
def auto_generate_and_publish_post():
    ...
    try:
        ...
        print(f"❌ [AUTOPOST] Ошибка публикации: {e}")
...
def home():
    ...
    if not _INITIALIZED:
        ...
        start_autoposting()
        _INITIALIZED = True
    return jsonify({"status": "running", "version": "yandex-gpt"}), 200
