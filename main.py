...
def show_catalog(message):
    try:
        ...
        # 🎛 Inline-кнопки — БЕЗ КНОПКИ "ПЕРЕЙТИ НА САЙТ"
        ...
        # 📦 Отправляем товары с красивыми иконками
        conn = sqlite3.connect(DB_PATH)
        cursor = conn.cursor()
        ...
        if not products:
            bot.send_message(message.chat.id, "📦 Каталог временно пуст.")
            return
        ...
        for product in products:
            ...
            # 🎯 Добавляем иконки к названию товара
            if "Рулонные шторы" in name:
                icon = "🧵"
            elif "Горизонтальные жалюзи" in name:
                icon = "🪟"
            elif "Вертикальные жалюзи" in name:
                icon = "🚪"
            elif "Жалюзи плиссе" in name:
                icon = "🌀"
            elif "Деревянные жалюзи" in name:
                icon = "🪵"
            else:
                icon = "🛍️"
