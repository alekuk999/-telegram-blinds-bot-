...
try:
    ...
    print("⚠️ Библиотека google-generativeai не установлена. Умные ответы отключены.")
...
GEMINI_API_KEY = os.getenv("GEMINI_API_KEY") if GEMINI_AVAILABLE else None
...
if GEMINI_AVAILABLE and GEMINI_API_KEY:
    ...
    print("✅ [GEMINI] API настроен и готов к работе.")
else:
    GEMINI_MODEL = None
    if GEMINI_AVAILABLE:
        print("⚠️ [GEMINI] Ключ не найден. Умные ответы отключены.")
...
def add_sample_data():
    ...
    if cursor.fetchone()[0] == 0:
        products = [
            ...
            (
                ...
                0.0,
                "Рулонные шторы",
                "https://via.placeholder.com/300x300?text=День-Ночь"
            ),
            (
                ...
                0.0,
                "Рулонные шторы",
                "https://via.placeholder.com/300x300?text=Зебра"
            ),
            (
                ...
                0.0,
                "Рулонные шторы",
                "https://via.placeholder.com/300x300?text=Блэкаут"
            ),
            ...
            (
                ...
                0.0,
                "Горизонтальные жалюзи",
                "https://via.placeholder.com/300x300?text=Алюминий"
            ),
            (
                ...
                0.0,
                "Горизонтальные жалюзи",
                "https://via.placeholder.com/300x300?text=Дерево"
            ),
            ...
            (
                ...
                0.0,
                "Вертикальные жалюзи",
                "https://via.placeholder.com/300x300?text=Ткань"
            ),
            (
                ...
                0.0,
                "Вертикальные жалюзи",
                "https://via.placeholder.com/300x300?text=ПВХ"
            ),
            ...
            (
                ...
                0.0,
                "Жалюзи плиссе",
                "https://via.placeholder.com/300x300?text=Плиссе+Ткань"
            ),
            (
                ...
                0.0,
                "Жалюзи плиссе",
                "https://via.placeholder.com/300x300?text=Плиссе+Алюминий"
            ),
            ...
            (
                ...
                0.0,
                "Деревянные жалюзи",
                "https://via.placeholder.com/300x300?text=Дуб"
            ),
            (
                ...
                0.0,
                "Деревянные жалюзи",
                "https://via.placeholder.com/300x300?text=Орех"
            )
        ]
    ...
    if cursor.fetchone()[0] == 0:
        smm_content = [
            ("5 причин выбрать рулонные шторы", "Рулонные шторы - идеальное решение для современного интерьера.", "https://via.placeholder.com/600x300?text=Рулонные+шторы", "Образование", None, 1, None),
            ("Как выбрать жалюзи для кухни", "Кухня требует особого подхода к выбору жалюзи.", "https://via.placeholder.com/600x300?text=Жалюзи+для+кухни", "Советы", None, 1, None),
            ("Тренды 2024", "Следите за модой в мире штор!", "https://via.placeholder.com/600x300?text=Тренды+2024", "Тренды", None, 1, None),
        ]
        cursor.executemany("INSERT INTO smm_content (title, content, image_url, category, scheduled_time, is_published, weekday) VALUES (?, ?, ?, ?, ?, ?, ?)", smm_content)
...
def ask_gemini(user_question: str) -> str:
    if not GEMINI_MODEL:
        return "🤖 Извините, умный помощник временно недоступен. Напишите менеджеру — мы на связи!"

    SYSTEM_PROMPT = """
    Ты — дружелюбный менеджер компании «Астра-Шторы» в Астрахани. Ты продаешь жалюзи и рулонные шторы на заказ.

    Условия работы:
    ✅ Бесплатный замер по всему городу и пригороду Астрахани
    ✅ Установка за 1 день
    ✅ Гарантия 3 года на изделия и монтаж
    ✅ Оплата после установки — наличные, карта, рассрочка

    Популярные товары и цены:
    - Рулонные шторы "День-Ночь" — от 2290 руб
    - Рулонные шторы "Блэкаут" — от 2790 руб
    - Горизонтальные жалюзи (алюминий) — от 1890 руб
    - Вертикальные жалюзи (ткань/ПВХ) — от 2100 руб

    Частые вопросы:
    — Сколько стоит установка? → Бесплатно!
    — Сколько ждать заказ? → От 1 до 3 дней
    — Гарантия? → 3 года
    — Работаете по районам? → Да, весь город: Советский, Кировский, Трусовский, Лиман и др.

    Твои цели:
    - Отвечать коротко, дружелюбно, с эмодзи
    - Предлагать оставить заявку, если клиент интересуется ценой или замером
    - Не обещать то, чего нет в промпте

    Если клиент спрашивает про замер — проси имя, телефон и удобное время.
    Если вопрос сложный или не по теме — ответь: “Передам ваш вопрос менеджеру — ждите звонка в течение 15 минут! 📞”
    """
    ...
    try:
        ...
        return "🤔 Пока не могу ответить, но передам менеджеру! Ожидайте звонка в течение 15 минут. 📞"
...
def handle_category_selection(call):
    try:
        ...
        for product in products:
            product_id, name, desc, image_url = product

            markup = types.InlineKeyboardMarkup()
            markup.add(
                types.InlineKeyboardButton("🔍 Подробнее", callback_data=f"details_{product_id}"),
                types.InlineKeyboardButton("🛒 Заказать", callback_data=f"order_{product_id}")
            )
...
def handle_details_button(call):
    try:
        ...
        bot.send_message(call.message.chat.id, "❌ Некорректный ID товара.")
    except Exception as e:
...
def handle_order_button(call):
    try:
        ...
        bot.send_message(
            ...
            f"🛒 Вы выбрали товар ID {product_id}.\n\n"
...
# === Автопостинг в канал по дням недели ===
...
# === 🤖 Универсальный обработчик для Gemini ===
@bot.message_handler(func=lambda m: True, content_types=['text'])
def handle_any_message(message):
    """
    Обрабатывает любые текстовые сообщения, не попавшие под другие команды.
    Передаёт их Gemini для генерации умного ответа.
    """
    try:
        save_user(message.from_user)
        save_message(message.from_user.id, message.text, True)
        ...
        if message.text in [
            ...
            "🔗 Канал", "💬 WhatsApp", "ℹ️ Помощь"
        ]:
            return  # Эти сообщения уже обрабатываются другими хендлерами

        # Запрашиваем ответ у Gemini
        bot_reply = ask_gemini(message.text)

        # Отправляем ответ
        bot.reply_to(message, bot_reply, parse_mode='Markdown')

        # Логируем
        print(f"💬 [GEMINI] Вопрос: {message.text}")
        print(f"🤖 [GEMINI] Ответ: {bot_reply}")

    except Exception as e:
        print(f"❌ [ANY MESSAGE] Ошибка: {e}")
        bot.reply_to(message, "❌ Произошла ошибка. Попробуйте позже.")
...
def home():
    ...
    if not _INITIALIZED:
        ...
        start_autoposting()
        _INITIALIZED = True
    return jsonify({"status": "running", "version": "2.3 + Gemini"}), 200
