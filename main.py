...
# === 🔥 ГЛАВНОЕ ИСПРАВЛЕНИЕ: Обработчик для /webhook (без токена в пути) ===
@app.route('/webhook', methods=['POST'])
def webhook():
    """
    Этот эндпоинт принимает POST-запросы от Telegram API.
    Telegram отправляет обновления сюда — на путь /webhook.
    """
...
# === Альтернативный путь с токеном (если решите вернуться к нему позже) ===
...
# === Установка вебхука ===
def set_webhook():
    # 🔥 Важно: Устанавливаем вебхук на /webhook, а не на /webhook/{token}
    webhook_url = f"https://telegram-blinds-bot-1.onrender.com/webhook"
    try:
        bot.remove_webhook()
        time.sleep(1)  # Небольшая пауза перед установкой нового
...
def initialize():
    init_db()
    add_sample_data()
    set_webhook()  # Устанавливаем вебхук на правильный URL
    start_autoposting()
