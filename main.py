...
# === Прочие обработчики (контакты, WhatsApp и т.д.) ===
...
def send_help(message):
    bot.reply_to(message, "📌 *Доступные функции бота*:\n\n• *Каталог* — посмотреть товары\n• *Заказать* — оставить заявку\n• *Контакты* — узнать адрес и телефон\n• *Канал* — подписаться на новости\n• *WhatsApp* — написать мгновенно", parse_mode='Markdown')

# === Заказ звонка ===
@bot.callback_query_handler(func=lambda call: call.data == "request_call")
def request_call_handler(call):
    bot.answer_callback_query(call.id)
    msg = bot.send_message(call.message.chat.id, "📞 *Отправьте ваш номер телефона:*", parse_mode='Markdown', reply_markup=types.ReplyKeyboardMarkup([[types.KeyboardButton("📲 Отправить мой номер", request_contact=True)]], resize_keyboard=True, one_time_keyboard=True))
...
def process_phone_number(message, user_name):
    ...
    bot.send_message(message.chat.id, f"✅ Спасибо, {user_name}! Менеджер перезвонит вам в течение 5 минут!", parse_mode='Markdown', reply_markup=types.ReplyKeyboardRemove())
...
def send_scheduled_posts():
    while True:
        try:
            ...
            conn = sqlite3.connect(DB_PATH)
            cursor = conn.cursor()
            cursor.execute("SELECT id, title, content, image_url, category FROM smm_content WHERE is_published = 0 AND scheduled_time <= ? ORDER BY scheduled_time LIMIT 1", (now,))
            row = cursor.fetchone()
            if row:
