...
def test_post(message):
    try:
        auto_generate_and_publish_post()
        bot.reply_to(message, "✅ Тестовый пост отправлен в канал @astra_jaluzi!")
    except Exception as e:
        bot.reply_to(message, f"❌ Ошибка при публикации: {str(e)}")
...
def handle_category_selection(call):
    ...
    descriptions = {
        "Рулонные шторы": "✨ *Рулонные шторы* — современное решение для любого интерьера:\n• День-ночь, зебра, блэкаут\n• Под заказ по вашим размерам\n• Установка за 1 день",
        "Горизонтальные жалюзи": "🪟 *Горизонтальные жалюзи* — классика надёжности:\n• Алюминиевые и деревянные\n• Не боятся влаги и пара\n• Идеальны для кухни и ванной",
        "Вертикальные жалюзи": "🚪 *Вертикальные жалюзи* — элегантность и функциональность:\n• Тканевые и ПВХ\n• Легко управлять поворотом\n• Подходят для больших окон и дверей",
        "Жалюзи плиссе": "🌀 *Жалюзи плиссе* — для нестандартных окон:\n• Мансарды, эркеры, арки\n• Мягкое рассеивание света\n• Ручное или автоматическое управление",
        "Деревянные жалюзи": "🪵 *Деревянные жалюзи* — натуральность и статус:\n• Дуб, орех, бук\n• Служат 15+ лет\n• Подчёркивают премиум-интерьер"
    }
