...
# === 🆕 НОВЫЙ ОБРАБОТЧИК: "ТКАНИ" ===
@bot.message_handler(func=lambda m: m.text == "🧵 Ткани")
def show_fabrics(message):
    """Показывает информацию о тканях и предлагает связаться."""
    try:
        save_message(message.from_user.id, message.text, True)
        
        text = (
            "🧵 *Широкий выбор тканей для рулонных штор и жалюзи!*\n\n"
            "У нас вы найдете:\n"
            "• *День-ночь* — чередование прозрачных и плотных полос\n"
            "• *Зебра* — современный дизайн с эффектом зебры\n"
            "• *Блэкаут* — 100% затемнение для комфортного сна\n"
            "• *Тканевые вертикальные жалюзи* — уют и элегантность\n"
            "• *ПВХ жалюзи* — практично для кухни и ванной\n\n"
            "✨ *Хотите посмотреть каталог тканей или подобрать материал под ваш интерьер?*\n"
            "Свяжитесь с нами удобным способом:"
        )
        
        markup = types.InlineKeyboardMarkup()
        markup.add(types.InlineKeyboardButton("💬 Написать в WhatsApp", url="https://wa.me/79378222906"))
        markup.add(types.InlineKeyboardButton("✉️ Написать в Telegram", url="https://t.me/astra_jalyzi30"))
        markup.add(types.InlineKeyboardButton("📞 Заказать звонок", callback_data="request_call"))
        
        bot.reply_to(message, text, parse_mode='Markdown', reply_markup=markup)
        print(f"✅ [BOT] Информация о тканях отправлена пользователю {message.from_user.id}")
    except Exception as e:
        print(f"❌ [BOT] Ошибка в обработчике 'Ткани': {e}")
