...
# 🎯 КАНАЛ ИСПРАВЛЕН НА: @astra_jaluzi
...
def add_sample_data():
    ...
    if cursor.fetchone()[0] == 0:
        smm_content = [
            ("5 причин выбрать рулонные шторы", "Рулонные шторы - идеальное решение для современного интерьера.", "images/rulonnye_zebra.jpg", "Образование", None, 1, None),
            ("Как выбрать жалюзи для кухни", "Кухня требует особого подхода к выбору жалюзи.", "images/gorizontalnye_aluminievye.jpg", "Советы", None, 1, None),
            ("Тренды 2024", "Следите за модой в мире штор!", "images/rulonnye_blackout.jpg", "Тренды", None, 1, None),
        ]
...
def generate_smm_post_with_ai(topic):
    """Генерирует пост для Telegram-канала через OpenAI GPT-4"""
    system_prompt = f"""
Ты — маркетолог в Астрахани, продаёшь жалюзи и рулонные шторы.
Твоя задача — создавать короткие, цепляющие посты для Telegram-канала {CHANNEL_ID}.
Название канала: “РУЛОННЫЕ ШТОРЫ ЖАЛЮЗИ АСТРАХАНЬ” — упоминай его в постах.

Формат:
- Заголовок (до 8 слов, с эмодзи)
- Текст (до 400 символов, с разбивкой на абзацы, 1–3 эмодзи)
- Призыв к действию: “Пишите в ЛС”, “Звоните”, “Успейте до...”
...
"""
    ...
    try:
        ...
        logging.error(f"❌ Ошибка генерации поста через AI: {e}")
        return None

def save_ai_generated_post(title, content, category="AI-генерация"):
    """Сохраняет сгенерированный пост в БД"""
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    try:
        cursor.execute('''
            INSERT INTO smm_content (title, content, category, scheduled_time, is_published, weekday)
            VALUES (?, ?, ?, ?, ?, ?)
        ''', (
            title[:100],
            content[:1000],
        ...
        conn.commit()
        logging.info(f"✅ [AI] Пост сохранён в БД: {title}")
    except Exception as e:
        logging.error(f"❌ Ошибка сохранения AI-поста: {e}")
    finally:
        conn.close()

def publish_post_to_channel(title, content, image_url=None):
    """Публикует пост в канал, с фото или без"""
    message = f"📌 <b>{title}</b>\n\n{content}"
    try:
        ...
        logging.info(f"✅ [PUBLISH] Пост опубликован в {CHANNEL_ID}: {title}")
        return True
    except Exception as e:
        logging.error(f"❌ [PUBLISH] Ошибка публикации в {CHANNEL_ID}: {e}")
        return False

def ai_autoposting_worker():
    """Фоновый процесс: генерирует и публикует 1 пост в день в 9:00"""
    logging.info(f"🤖 [AI AUTOPOST] Запущен фоновый процесс AI-автопостинга для канала {CHANNEL_ID}")
        ...
        try:
            ...
            if now.time() >= dt_time(9, 0):
                topics = [
                    ...
                    "Защита от астраханской жары с рулонными шторами",
                    "Тренды 2025 в мире жалюзи",
                    "Почему выбирают деревянные жалюзи?",
                ...
                logging.info(f"🧠 [AI] Генерирую пост на тему: {topic}")
                ai_post = generate_smm_post_with_ai(topic)

                if ai_post:
                    success = publish_post_to_channel(f"🔥 {topic}", ai_post)
                    if success:
                        save_ai_generated_post(topic, ai_post)
                        last_post_date = current_date
                        logging.info(f"🎉 [AI] Успешно опубликован и сохранён пост на тему: {topic}")
                else:
                    logging.warning("⚠️ [AI] Не удалось сгенерировать пост")

            time.sleep(60)

        except Exception as e:
            logging.error(f"❌ [AI AUTOPOST] Критическая ошибка: {e}")
            time.sleep(60)

def start_ai_autoposting():
    thread = threading.Thread(target=ai_autoposting_worker, daemon=True)
    thread.start()
    logging.info("🧵 [AI AUTOPOST] AI-автопостинг запущен как фоновый поток")
...
# === Автопостинг в канал по дням недели (ОСТАВЛЕН ДЛЯ СТАРЫХ ПОСТОВ) ===
def send_scheduled_posts():
    print("⏱️ [AUTOPOST] Задача автопостинга (ручные посты) запущена")
    while True:
        try:
            now = datetime.now()
...
def start_legacy_autoposting():
    thread = threading.Thread(target=send_scheduled_posts, daemon=True)
    thread.start()
    print("🧵 [AUTOPOST] Легаси-автопостинг запущен как фоновый поток")
...
def home():
    ...
    if not _INITIALIZED:
        ...
        add_sample_data()
        set_webhook()
        start_legacy_autoposting()    # Для старых постов из БД
        start_ai_autoposting()        # 🔥 ДЛЯ НОВЫХ AI-ПОСТОВ
        _INITIALIZED = True
    return jsonify({"status": "running", "version": "2.6-AI-INTEGRATED", "channel": CHANNEL_ID}), 200
