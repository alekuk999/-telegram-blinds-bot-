...
# === Отключаем предупреждение pkg_resources (устраняем warning) ===
...
# Render задаёт порт автоматически
PORT = int(os.getenv("PORT", 10000))
WEBHOOK_URL = os.getenv("WEBHOOK_URL", f"https://telegram-blinds-bot-1.onrender.com")
...
# Остальные обработчики (без изменений) — можно оставить как есть
...
# === Главная страница (для мониторинга) ===
@app.route('/')
def home():
    return jsonify({
        "status": "running",
...
# === API эндпоинты (оставьте как есть) ===
@app.route('/api/stats')
def get_stats():
    # ... ваш код
    pass
...
# === Запуск (Render использует Gunicorn, не этот блок) ===
