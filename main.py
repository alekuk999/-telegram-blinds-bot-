...
def request_call_handler(call):
    try:
        user = call.from_user
        phone_number = "+7 (937) 822-29-06"

        # ✅ Подтверждение для пользователя
        bot.answer_callback_query(call.id, text="📞 Мы перезвоним вам в течение 5 минут!", show_alert=True)
        ...
        bot.send_message(
            ...
            f"📞 *Телефон:* [{phone_number}](tel:+79378222906)\n"
            f"💬 *WhatsApp:* [Написать сейчас](https://wa.me/+79378222906)\n\n"
            f"⏳ *Ожидайте звонка!*",
            parse_mode='Markdown',
            reply_markup=types.InlineKeyboardMarkup([
                [types.InlineKeyboardButton("📲 Позвонить сейчас", url=f"tel:+79378222906")],
                [types.InlineKeyboardButton("💬 Написать в WhatsApp", url="https://wa.me/+79378222906")]
            ])
        )

        # 📊 Опционально: сохраните заявку в базу данных
        save_call_request(user.id, user.first_name)

    except Exception as e:
        print(f"❌ Ошибка в request_call_handler: {e}")


def save_call_request(user_id, first_name):
    ...
    try:
        cursor.execute('''
            INSERT INTO messages (user_id, message, is_from_user)
            VALUES (?, ?, ?)
        ''', (user_id, f"Запрос на обратный звонок от {first_name}", False))
        conn.commit()
        print(f"📞 [CALL REQUEST] Заявка от {first_name} (ID: {user_id}) сохранена.")
    except Exception as e:
