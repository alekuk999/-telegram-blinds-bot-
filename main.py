...
def query_yandex_gpt(user_question):
    ...
    try:
        ...
        return "⚠️ Не удалось получить ответ. Попробуйте позже или свяжитесь с менеджером."
...
# === 👨‍💼 Спросить у менеджера (НОВОЕ) ===
@bot.message_handler(func=lambda m: m.text == "👨‍💼 Спросить у менеджера")
def start_manager_or_ai_chat(message):
    save_message(message.from_user.id, message.text, True)
    bot.send_message(
        message.chat.id,
        "💬 Здравствуйте! Сначала вам ответит наш AI-помощник по жалюзи и шторам.\n\n"
        "Если вопрос сложный или вы хотите поговорить с человеком — просто напишите:\n"
        "*«Хочу менеджера»*, *«Свяжитесь со мной»* или *«Перезвоните»*.\n\n"
        "Задайте свой вопрос:",
        parse_mode='Markdown'
    )
...
def handle_ai_or_forward_to_manager(message):
    ...
    trigger_phrases = ["менеджер", "человек", "связаться", "перезвоните", "позвоните", "оператор", "хочу менеджера", "свяжитесь", "перезвонить"]
    if any(phrase in user_text.lower() for phrase in trigger_phrases):
        try:
            bot.send_message(
                MANAGER_CHAT_ID,
                f"🔔 *Запрос на связь с менеджером!*\n\n"
            ...
            bot.send_message(
                message.chat.id,
                "✅ Отлично! Менеджер скоро свяжется с вами. Обычно это занимает до 5 минут.\n\n"
                "А пока можете посмотреть наш 📚 *Каталог* или написать в 💬 *WhatsApp*.",
                parse_mode='Markdown',
                reply_markup=types.ReplyKeyboardRemove()
            )
            show_main_menu(message)
            ...
            print(f"❌ Ошибка уведомления менеджера: {e}")
            bot.send_message(message.chat.id, "⚠️ Не удалось передать запрос. Напишите, пожалуйста, напрямую в WhatsApp: https://wa.me/79378222906")
        return

    save_message(message.from_user.id, user_text, True)
    bot.send_chat_action(message.chat.id, 'typing')
    answer = query_yandex_gpt(user_text)
    save_message(message.from_user.id, answer, False)
...
def continue_ai_or_manager_flow(message):
    if message.text == "🔄 Задать ещё вопрос":
        ...
        bot.send_message(message.chat.id, "📞 Хорошо! Напишите, пожалуйста, ваш вопрос или номер телефона:")
...
def forward_to_manager(message):
    try:
        bot.send_message(
            MANAGER_CHAT_ID,
            f"🔔 *Пользователь запросил связь с менеджером!*\n\n"
        ...
        bot.send_message(
            ...
            "Также вы можете написать напрямую: [WhatsApp](https://wa.me/79378222906)",
            parse_mode='Markdown'
        )
        show_main_menu(message)
    except Exception as e:
        bot.send_message(message.chat.id, "❌ Не удалось отправить. Пожалуйста, напишите в WhatsApp.")
        show_main_menu(message)
...
def home():
    ...
    if not _INITIALIZED:
        ...
        start_autoposting()
        _INITIALIZED = True
    return jsonify({"status": "running", "version": "5.1"}), 200
