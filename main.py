...
MANAGER_CHAT_ID = 7126605143  # Убедитесь, что это число
    ...
    if cursor.fetchone()[0] == 0:
        products = [
            (
                ...
                "✨ *Идеально для спальни и гостиной!* Чередование прозрачных и плотных полос — регулируйте свет без подъёма шторы.",
                "Рулонные шторы",
                "https://img.freepik.com/free-photo/modern-living-room-interior-design_1268-16720.jpg"
            ),
            (
                "Горизонтальные жалюзи алюминиевые",
                "🔧 *Классика, проверенная временем!* Алюминиевые ламели 25 мм — лёгкие, прочные, не ржавеют.",
                "Горизонтальные жалюзи",
                "https://img.freepik.com/free-photo/vertical-blinds-window_1268-17953.jpg"
            ),
            (
                "Вертикальные жалюзи тканевые",
                "🌿 *Элегантность и уют!* Плотный полиэстер — не выгорает, поглощает шум, создаёт атмосферу простора.",
                "Вертикальные жалюзи",
...
def generate_post_with_yandex_gpt(product_name, product_description):
    ...
    headers = {
        ...
        "Authorization": f"Api-Key {YANDEX_API_KEY}"  # ← ПРАВИЛЬНО!
    }
    prompt = f"""
