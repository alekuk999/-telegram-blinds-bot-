...
def start_autoposting():
    ...
    thread.start()
    logger.info("🧵 [AUTOPOST] Автопостинг запущен")

# === Обработчики команд ===
@bot.message_handler(commands=['start'])
def send_welcome(message):
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True, row_width=2)
    markup.add("📚 Каталог", "📞 Контакты")
    markup.add("💬 WhatsApp", "🛒 Заказать")
...
def home():
    ...
    if not _INITIALIZED:
        ...
        start_autoposting()
        _INITIALIZED = True
    return jsonify({"status": "running", "channel": "astra_jaluzi"}), 200

def set_webhook():
    # 🔥 Обязательно укажите ваш домен Timeweb
