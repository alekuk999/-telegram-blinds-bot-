...
def add_sample_data():
    ...
    if cursor.fetchone()[0] == 0:
        print("ℹ️ [DB] Таблица smm_content пуста — посты будут генерироваться через OpenAI GPT.")
...
# === 🆕 Функция для поиска и скачивания фото ===
def search_and_download_image(query, filename):
    """Ищет изображение в Яндекс.Картинках и скачивает его."""
    try:
        # Настройка Selenium
        ...
        # Поиск на Яндекс.Картинках
        search_url = f"https://yandex.ru/images/search?text={query}"
        driver.get(search_url)
        time.sleep(3)

        # Находим первое изображение
        image_element = driver.find_element(By.CSS_SELECTOR, "img.serp-item__thumb")
        ...
        if response.status_code == 200:
            ...
            print(f"✅ Фото скачано: {filename}")
            driver.quit()
            return filename
        else:
            print(f"❌ Не удалось скачать фото: {image_url}")
            driver.quit()
            return None
...
# === 🆕 Функция генерации поста с помощью OpenAI GPT ===
def auto_generate_and_publish_post():
    """Полностью автоматическая генерация и публикация поста."""
    try:
        # Генерируем тему для поста с помощью OpenAI
        prompt = """
        Ты — маркетолог интернет-магазина жалюзи в Астрахани.
        Придумай тему для поста в Telegram-канале. Это может быть:
        - Совет по выбору жалюзи
        - Тренд сезона
        - Описание конкретного типа жалюзи
        - Акция или спецпредложение

        Ответь одной строкой — только тему, без лишних слов.
        """
        ...
        print(f"🧠 Сгенерирована тема: {topic}")

        # Ищем и скачиваем фото
        image_filename = f"images/auto_{int(time.time())}.jpg"
        downloaded_image = search_and_download_image(topic, image_filename)

        if not downloaded_image:
            print("❌ Не удалось найти фото. Используем заглушку.")
            # Создаем файл-заглушку, если его нет
            if not os.path.exists("images/placeholder.jpg"):
                with open("images/placeholder.jpg", "w") as f:
                    f.write("")
            downloaded_image = "images/placeholder.jpg"

        # Генерируем описание поста
        description_prompt = f"""
        Напиши описание для поста в Telegram на тему: "{topic}".
        Текст должен быть:
        - Кратким (не более 150 символов)
        - Цепляющим, с эмодзи
        - Содержать призыв к действию (написать в WhatsApp или заказать звонок)
        - Включать 3 релевантных хэштега

        Ответь только текстом поста, без форматирования.
        """
        ...
        print(f"📝 Сгенерировано описание: {post_content}")

        # Публикуем пост в канал
        with open(downloaded_image, 'rb') as photo_file:
            bot.send_photo(CHANNEL_ID, photo_file, caption=post_content, parse_mode='Markdown')

        print(f"✅ Пост опубликован: {topic}")
