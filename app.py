...
def send_welcome(message):
    try:
        ...
        print(f"✅ [BOT] Приветственное сообщение отправлено пользователю {message.from_user.id}")
    except Exception as e:
...
def send_help(message):
    try:
        print(f"🤖 [BOT] Обработчик /help вызван для пользователя {message.from_user.id}")
        save_message(message.from_user.id, message.text, True)
        ...
        print(f"❌ [BOT] Ошибка в обработчике /help: {e}")
        import traceback
        traceback.print_exc()
...
def show_catalog(message):
    try:
        ...
        if products:
            ...
            print(f"✅ [BOT] Сообщение 'Каталог пуст' отправлено пользователю {message.from_user.id}")
    except Exception as e:
